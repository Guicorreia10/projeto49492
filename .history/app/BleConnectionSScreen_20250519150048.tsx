...
export default function BLEConnectionsScreen() {
  ...
  const loadHistory = async () => {
    try {
      const json = await AsyncStorage.getItem('ble_history');
  ...
  // Atualiza histórico e guarda em AsyncStorage
  const saveToHistory = async (device: Device) => {
    const entry = { id: device.id, name: device.name || null };
    const newHistory = [entry, ...history.filter(h => h.id !== device.id)].slice(0, 10);
    setHistory(newHistory);
    try {
      await AsyncStorage.setItem('ble_history', JSON.stringify(newHistory));
    } catch (e) {
    ...
    try {
      ...
      const readValue = async (charUUID: string) => {
        try {
          ...
          console.warn(`Characteristic ${charUUID} not available or read error:`, e.message);
          return 0;
        }
      };
      ...
      Alert.alert(
        'Aparelho conectado',
        `Conectado a ${device.name || device.id}
Passos: ${passos}, Sono: ${sono}, Exercício: ${exercicio}`
      );
    } catch (err: any) {
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.toggleRow}>
        <TouchableOpacity
          style={[styles.toggleButton, view==='scan' && styles.toggleActive]}
          onPress={() => setView('scan')}
        >
          <Text style={styles.toggleText}>Scan</Text>
        </TouchableOpacity>
        <TouchableOpacity
          style={[styles.toggleButton, view==='history' && styles.toggleActive]}
          onPress={() => setView('history')}
        >
          <Text style={styles.toggleText}>Histórico</Text>
        </TouchableOpacity>
      </View>
      ...
      {connectedDevice && (
        <Text style={styles.connected}>
          Aparelho conectado: {connectedDevice.name || connectedDevice.id}
        </Text>
      )}
      ...
      {view === 'scan' ? (
        ...
        <FlatList
          ...
          keyExtractor={item => item.id}
          renderItem={({ item }) => (
            <TouchableOpacity style={styles.item} onPress={() => handleConnectAndFetch(item as any)}>
              <Text style={styles.itemText}>{item.name || item.id}</Text>
              <Text style={styles.itemId}>{item.id}</Text>
            </TouchableOpacity>
          )}
          ListEmptyComponent={() => <Text style={styles.empty}>Sem histórico</Text>}
        />
      )}
      ...
      <View style={styles.buttonsRow}>
        <TouchableOpacity
          ...
          onPress={startScan}
          disabled={isScanning || view!=='scan'}
        >
          <Text style={styles.scanText}>Iniciar Scan</n```) => {
      stopScan();
      managerRef.current.destroy();
    };
  }, []);
