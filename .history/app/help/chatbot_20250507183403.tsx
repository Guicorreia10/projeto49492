...
export default function Chatbot() {
  ...
  const [history, setHistory] = useState<RoleMessage[]>([
    { role: 'system', content: 'És o GlicoBot, um assistente amigável especializado em sono, glicose e bem-estar.' },
  ]);
  ...
  const sendMessage = useCallback(async () => {
    ...
    const newHistory = [...history, { role: 'user', content: input } as RoleMessage];
    setHistory(newHistory);
    setInput('');
    setLoading(true);

    // chama API com todo o contexto
    try {
      ...
      const res = await fetch('https://openrouter.ai/api/v1/chat/completions', {
        ...
        headers: {
          ...
          Authorization: 'Bearer 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',
        },
        body: JSON.stringify({
      ...
      const botContent = json.choices?.[0]?.message?.content?.trim()
        || 'Desculpa, não consegui gerar uma resposta.';
      setHistory(prev => [...prev, { role: 'assistant', content: botContent } as RoleMessage]);

      setMessages(prev => [...prev, { id: Date.now().toString(), text: botContent, fromBot: true }]);
    } catch (e) {
      console.error('❌ Erro ao contactar a API:', e);
      setMessages(prev => [...prev, { id: Date.now().toString(), text: 'Erro ao comunicar com o servidor.', fromBot: true }]);
    } finally {
      setLoading(false);
    }
  }, [input, history]);
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      {loading && <ActivityIndicator style={styles.loading} size="small" color="#4A90E2" />}
...
const styles = StyleSheet.create({
  ...
  botBubble: { backgroundColor: '#F1F1F1', alignSelf: 'flex-start', borderBottomLeftRadius: 0 },
  userText: { color: '#fff' }, botText: { color: '#333' },
  inputRow: { flexDirection: 'row', padding: 8, borderTopWidth: 1, borderColor: '#eee' },
  input: { flex: 1, backgroundColor: '#F7F7F7', borderRadius: 20, paddingHorizontal: 16, fontSize: 16 },
  sendBtn: { marginLeft: 8, backgroundColor: '#4A90E2', borderRadius: 20, paddingVertical: 10, paddingHorizontal: 16 },
