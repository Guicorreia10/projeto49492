...
export default function BLEConnectionsScreen() {
  ...
  const onDeviceDiscovered = (error: BleError | null, device: Device | null) => {
    ...
    if (device && device.id) {
      console.log('Discovered device:', device.id, device.name); // Log device
  ...
  const handleConnectAndFetch = async (device: Device) => {
    ...
    try {
      ...
      const readValue = async (charUUID: string) => {
        ...
        console.log(`Characteristic ${charUUID} base64 value:`, char.value);
        ...
        try {
          const val = buf.readUIntLE(0, readLen);
          console.log(`Parsed value (${readLen} bytes):`, val);
          return val;
        } catch (e) {
