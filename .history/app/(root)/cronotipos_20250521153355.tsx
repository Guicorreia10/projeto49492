...
const CronotipoScreen = () => {
  ...
  const handleSelecionar = (perguntaId: number, valor: number) => {
  ...
  const calcularResultado = async () => {
    ...
    const score = Object.values(respostas).reduce((acc, val) => acc + val, 0);
        ...
        {perguntas.map((p) => (
          <View key={p.id} style={{ marginBottom: 24 }}>
            ...
            {p.opcoes.map((op, i) => (
              <TouchableOpacity
                key={i}
                style={[styles.botaoOpcao, respostas[p.id] === op.valor && styles.botaoSelecionado]}
                onPress={() => handleSelecionar(p.id, op.valor)}
              >
                <Text style={respostas[p.id] === op.valor ? styles.opcaoSelecionada : styles.opcaoTexto}>{op.texto}</Text>
              </TouchableOpacity>
            ))}
          </View>
        ))}

        <TouchableOpacity style={styles.botaoFinalizar} onPress={calcularResultado}>
          <Text style={styles.botaoFinalizarTexto}>Ver Resultado</Text>
        </TouchableOpacity>
