    ...
    const fetchSmartwatchData = async () => {
      try {
        ...
        const { data, error } = await supabase
          ...
          .select("passos, exercicio, sono")
          .eq("user_id", userId)
          .order("created_at", { ascending: false })
          .limit(1);

        if (error) {
          console.error("Erro ao buscar smartwatch:", error);
          return;
        }

        if (data && data.length > 0) {
          setSmartwatchData(data[0]);
        }
      } catch (err) {
        console.error("Erro inesperado smartwatch:", err);
      }
    };
        ...
        {smartwatchData && (
          <View style={styles.card}>
            ...
            <Text style={styles.cardText}>
              ...
              🏃 Exercício: {smartwatchData.exercicio ?? "N/D"} min{"\n"}
              💤 Sono: {smartwatchData.sono ?? "N/D"} h
            </Text>
          </View>
        )}
