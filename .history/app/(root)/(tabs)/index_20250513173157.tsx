...
export default function Index() {
  ...
  const [smartData, setSmartData] = useState({ passos: null, sono: null, exercicio: null });
    ...
    const fetchLatestData = async () => {
      try {
        ...
        const { data, error } = await supabase
          .from("dados_usuario")
          .select("sono, qualidade_sono, dificuldade_ao_dormir, uso_dispositivos, glicose, passos, exercicio")
          .eq("user_id", userId)
        ...
        if (data && data.length > 0) {
          const { sono, qualidade_sono, dificuldade_ao_dormir, uso_dispositivos, glicose, passos, exercicio } = data[0];

          setSmartData({ passos, sono, exercicio });
    ...
    <SafeAreaView style={styles.safeArea}>
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.card}>
          ...
          <Text style={styles.cardText}>Passos: {smartData.passos ?? 'A carregar...'}</Text>
          <Text style={styles.cardText}>Sono: {smartData.sono ?? 'A carregar...'} min</Text>
          <Text style={styles.cardText}>Exercício: {smartData.exercicio ?? 'A carregar...'} min</Text>
        </View>
