...
export default function AnaliseComida() {
  ...
  const analyzeImage = async (uri: string) => {
    ...
    try {
      // 1) Reconhecer nome do alimento
      const foodName = await recognizeFoodWithClarifai(uri);

      // 2) Buscar dados nutricionais
      ...
      if (insertErr) {
        console.warn('Aviso: não foi possível gravar no servidor.', insertErr);
      }
    } catch (err: any) {
      Alert.alert('Erro', err.message || 'Falha ao analisar a imagem.');
    } finally {
