...
export async function getRealNutritionData(foodName: string, quantity: string) {
  ...
  try {
    ...
    if (!data.ingredients || data.ingredients.length === 0) throw new Error('Dados nutricionais não encontrados.');

    const ingredientData = data.ingredients[0].parsed[0];
    const nutrients = ingredientData.nutrients;
    ...
    return {
      ...
      glycemicIndex: 50, // Placeholder, adicionar lógica de índice glicêmico, se necessário
      glycemicLoad: 'N/A', // Placeholder, adicionar cálculo de carga glicêmica, se necessário
      classification: 'N/A', // Placeholder
    };
  } catch (error) {
