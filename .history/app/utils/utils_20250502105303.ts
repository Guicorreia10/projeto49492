...
export async function recognizeFoodWithClarifai(imageUri: string): Promise<string> {
  try {
    ...
    const raw = JSON.stringify({
      ...
      inputs: [{ data: { image: { base64 } } }], // Envia a imagem base64
    });
    ...
    if (concepts && concepts.length > 0) {
      return concepts[0].name; // Retorna o nome do alimento reconhecido
    }
...
export async function getRealNutritionData(foodName: string, quantity: string) {
  ...
  try {
    ...
    // Verifica se os dados nutricionais estão disponíveis
    if (!data || !data.ingredients || data.ingredients.length === 0) {
      throw new Error('Dados nutricionais não encontrados para a quantidade informada.');
    }

    const ingredientData = data.ingredients[0].parsed[0].nutrients;

    // Verifica se os dados de carboidratos estão presentes e válidos
    if (!ingredientData || !ingredientData.CHOCDF || ingredientData.CHOCDF.quantity === 0) {
      throw new Error('Dados nutricionais incompletos ou inválidos.');
    }

    // Retorna os dados nutricionais válidos
    return {
      name: foodName,
      portion: `${quantity}g`,
      calories: ingredientData.ENERC_KCAL.quantity || 0,
      carbs: ingredientData.CHOCDF.quantity || 0,
    ...
    return null; // Retorna null em caso de erro, garantindo que o código continue funcionando
  }
}
