...
export async function gerarOpenRouterInsights(registos: Registo[]): Promise<string[]> {
  const apiKey = process.env.OPENROUTER_API_KEY; // Coloca isto no teu `.env` e garante que está no `.gitignore`
  ...
  try {
    const resposta = await fetch("https://openrouter.ai/api/v1/chat/completions", {
      ...
      headers: {
        ...
        "HTTP-Referer": "https://teu-site-ou-app.com", // opcional, mas ajuda com rate limits
      },
      body: JSON.stringify({
