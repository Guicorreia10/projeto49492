...
export async function recognizeFoodWithClarifai(imageUri: string): Promise<string> {
  // 1. Redimensiona e comprime a imagem ANTES de converter para base64
  const manipResult = await ImageManipulator.manipulateAsync(
    imageUri,
    [{ resize: { width: 800 } }], // Ajuste a largura se necessário (ex: 600)
    { compress: 0.5, format: ImageManipulator.SaveFormat.JPEG }
  );

  // 2. Converta para base64 a partir da imagem manipulada
