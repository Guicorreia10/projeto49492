  ...
  const mensagens = [
    {
      role: "system",
      content:
        "És um assistente de saúde inteligente. Com base nos registos fornecidos (sono, glicose, alimentação, medicamentos), gera insights personalizados, para melhorar a saúde metabólica do utilizador. Foca-te em padrões, correlações e sugestões realistas, fala sempre dos medicamentos que tomou.",
    },
    {
      role: "user",
  ...
  try {
    const resposta = await fetch("https://openrouter.ai/api/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: mensagens,
        temperature: 0.7,
         max_tokens: 1000, // reduzido para evitar respostas longas
      }),
    });
