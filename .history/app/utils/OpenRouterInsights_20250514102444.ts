...
export async function gerarOpenRouterInsights(registos: Registo[]): Promise<string[]> {
  ...
  const mensagens = [
    ...
    {
      role: "user",
      content: `Aqui estão os dados do utilizador (formato JSON):\n${JSON.stringify(registos, null, 2)}\n\nCria 5 insights personalizados em bullet points, devidamente explicativos e tendo sempre em conta a análise dos dados enviados, tem em atenção sempre a medicação, a alimentação e os valores de sono e glicose cria pelo menos um insight para cada opção`,
    },
  ];
