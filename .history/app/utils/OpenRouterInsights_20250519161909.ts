...
export async function gerarOpenRouterInsights(registos: Registo[]): Promise<string[]> {
  ...
  const mensagens = [
    ...
    {
      role: "user",
      content: `Aqui estão alguns dados do utilizador (formato JSON):\n${JSON.stringify(registos.slice(0, 20), null, 2)}\n\nCria 5 insights em bullet points acerca destes dados. Cada insight deve ser claro e explicativo e deve ser pelo menos um de cada tema (medicamento,glicose,sono,comida).`,
    },
  ];
  ...
  try {
    const resposta = await fetch("https://openrouter.ai/api/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: mensagens,
        temperature: 0.7,
         max_tokens: 300, // reduzido para evitar respostas longas
      }),
    });
